...
def scan_all_systems(username):
    while True:
        ...
        for system in systems:
            socketIOServer.emit("statusUpdate", room=username, data={"system": system, "statusText": "Started NMAP Scan", "mode": "Running"})
...
class MetasploitCannon(CannonPlug):
    ...
    def do_post_exploitation(self, session):
        ...
        if result == 'success':
            ...
            result = self.client.execute_meterpreter(session_id, "whoami")
            time.sleep(1)
            if(result != "Falied"):
